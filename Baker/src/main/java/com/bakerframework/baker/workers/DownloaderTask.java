...
public class DownloaderTask extends AsyncTask<String, Long, String> {
    ...
    public SharedPreferences getDownloadPreferences() {
        return this.context.getSharedPreferences(ShelfActivity.class.getSimpleName(), Context.MODE_PRIVATE);
    }

    private void storeDownloadId() {
        SharedPreferences preferences = context.getSharedPreferences(ShelfActivity.class.getSimpleName(), Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = preferences.edit();
        editor.putLong(Configuration.PREF_DOWNLOAD_IN_PROGRESS + this.fileName, this.downloadId);
        editor.commit();
    }

    private long restoreDownloadId() {
        SharedPreferences preferences = context.getSharedPreferences(ShelfActivity.class.getSimpleName(), Context.MODE_PRIVATE);
        return preferences.getLong(Configuration.PREF_DOWNLOAD_IN_PROGRESS + this.fileName, -1L);
    }
    ...
    private void removeDownloadId() {
        ...
        SharedPreferences preferences = context.getSharedPreferences(ShelfActivity.class.getSimpleName(), Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = preferences.edit();
        editor.remove(Configuration.PREF_DOWNLOAD_IN_PROGRESS + this.fileName);
        editor.commit();
    }
            ...
            while (downloading) {
                ...
                switch (status) {
                    case DownloadManager.STATUS_PAUSED:
                        if (!Configuration.hasNetworkConnection()) {
                            result = "ERROR";
